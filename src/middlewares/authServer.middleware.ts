...
const authServerMiddleware = async (req: RequestWithUser, res: Response, next: NextFunction) => {
  try {
    const Authorization = (req.header('Authorization') ? req.header('Authorization').split('Bearer ')[1] : null) || req.cookies['Authorization']
    if (Authorization) {

      // if(Authorization == 'Fr7vcLUmiVoayHrkNMU158z1v0AIUJ8bLm0Qx1mtGbIcQVaZ6j') {
      //   next();
      // } else {
      //   next(new HttpException(404, 'Can be use sever only'));
      // }
